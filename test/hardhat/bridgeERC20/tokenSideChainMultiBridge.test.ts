...
describe('TokenSideChainMultiBridge', () => {
  ...
  describe('initialize', () => {
    it('success - correctly initialized', async () => {
      ...
      ).to.be.revertedWithCustomError(angleRevert, 'ZeroAddress');
    });
  });
  describe('setCore', () => {
    it('reverts - non governor and incorrect address', async () => {
      await expect(angle.connect(alice).setCore(ZERO_ADDRESS)).to.be.revertedWithCustomError(angle, 'NotGovernor');
      ...
      await expect(angle.connect(deployer).setCore(newCoreBorrow.address)).to.be.revertedWithCustomError(
        angle,
        'NotGovernor',
      );
    });
  ...
  describe('addBridgeToken', () => {
    ...
    it('reverts - non governor', async () => {
      ...
      ).to.be.revertedWithCustomError(angle, 'NotGovernor');
    });
    it('reverts - too high parameter value', async () => {
      ...
      ).to.be.revertedWithCustomError(angle, 'TooHighParameterValue');
    });
    it('reverts - zero address', async () => {
      ...
      ).to.be.revertedWithCustomError(angle, 'InvalidToken');
    });
  ...
  describe('removeBridgeToken', () => {
    it('reverts - non governor', async () => {
      await expect(angle.connect(bob).removeBridgeToken(bridgeToken.address)).to.be.revertedWithCustomError(
        angle,
        'NotGovernor',
      );
    });
    it('reverts - non null balance', async () => {
      ...
      await expect(angle.connect(deployer).removeBridgeToken(bridgeToken.address)).to.be.reverted;
    });
  ...
  describe('setLimit', () => {
    it('reverts - non governor and non guardian and non keeper', async () => {
      await expect(angle.connect(alice).setLimit(bridgeToken.address, parseEther('1'))).to.be.revertedWithCustomError(
        angle,
        'NotGovernorOrGuardian',
      );
    });
    it('reverts - non allowed token', async () => {
      await expect(angle.connect(deployer).setLimit(alice.address, parseEther('1'))).to.be.revertedWithCustomError(
        angle,
        'InvalidToken',
      );
    });
  ...
  describe('setHourlyLimit', () => {
    it('reverts - non governor and non guardian and non keeper', async () => {
      await expect(
        angle.connect(alice).setHourlyLimit(bridgeToken.address, parseEther('1')),
      ).to.be.revertedWithCustomError(angle, 'NotGovernorOrGuardian');
    });
    it('reverts - non allowed token', async () => {
      await expect(
        angle.connect(deployer).setHourlyLimit(alice.address, parseEther('1')),
      ).to.be.revertedWithCustomError(angle, 'InvalidToken');
    });
    it('success - value updated', async () => {
      const receipt = await (
  ...
  describe('setChainTotalHourlyLimit', () => {
    it('reverts - non governor and non guardian and non keeper', async () => {
      await expect(angle.connect(alice).setChainTotalHourlyLimit(parseEther('1'))).to.be.revertedWithCustomError(
        angle,
        'NotGovernorOrGuardian',
      );
    });
  ...
  describe('setSwapFee', () => {
    it('reverts - non governor and non guardian', async () => {
      await expect(
        angle.connect(alice).setSwapFee(bridgeToken.address, parseAmount.gwei('0.5')),
      ).to.be.revertedWithCustomError(angle, 'NotGovernorOrGuardian');
    });
    it('reverts - non allowed token', async () => {
      await expect(
        angle.connect(deployer).setSwapFee(alice.address, parseAmount.gwei('0.5')),
      ).to.be.revertedWithCustomError(angle, 'InvalidToken');
    });
    it('reverts - too high value', async () => {
      await expect(
        angle.connect(deployer).setSwapFee(bridgeToken.address, parseAmount.gwei('2')),
      ).to.be.revertedWithCustomError(angle, 'TooHighParameterValue');
    });
    it('success - value updated', async () => {
      const receipt = await (
  ...
  describe('toggleBridge', () => {
    it('reverts - non governor and non guardian', async () => {
      await expect(angle.connect(alice).toggleBridge(bridgeToken.address)).to.be.revertedWithCustomError(
        angle,
        'NotGovernorOrGuardian',
      );
    });
    it('reverts - non existing bridge', async () => {
      await expect(angle.connect(deployer).toggleBridge(alice.address)).to.be.revertedWithCustomError(
        angle,
        'InvalidToken',
      );
    });
  ...
  describe('toggleFeesForAddress', () => {
    it('reverts - non governor and non guardian', async () => {
      await expect(angle.connect(alice).toggleFeesForAddress(bridgeToken.address)).to.be.revertedWithCustomError(
        angle,
        'NotGovernorOrGuardian',
      );
    });
  ...
  describe('swapIn', () => {
    it('reverts - incorrect bridge token', async () => {
      await expect(
        angle.connect(deployer).swapIn(bob.address, parseEther('1'), alice.address),
      ).to.be.revertedWithCustomError(angle, 'InvalidToken');
    });
  ...
  describe('swapOut', () => {
    ...
    it('reverts - incorrect bridge token', async () => {
      await expect(
        angle.connect(deployer).swapOut(bob.address, parseEther('1'), alice.address),
      ).to.be.revertedWithCustomError(angle, 'InvalidToken');
    });
    it('reverts - bridge token paused', async () => {
      await angle.connect(deployer).toggleBridge(bridgeToken.address);
    ...
    it('reverts - hourly limit exceeded', async () => {
      ...
      ).to.be.revertedWithCustomError(angle, 'HourlyLimitExceeded');
    });
    ...
    it('success - hourly limit updated by governance', async () => {
      ...
      ).to.revertedWithCustomError(angle, 'HourlyLimitExceeded');
